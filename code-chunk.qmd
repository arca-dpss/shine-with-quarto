...
The code chunks are the most brilliant thing about quarto (and RMarkdown): they allow for including actual code within a document and to carry out the specified analysis while the document is compiled. Long story short: This feature allows for having the report/article, table, graphs, data, and the code used to obtain all the above mentioned elements in one unique document.
...
Differently from `eval: false` (i.e., the code is not executed, if there are errors in the code you will not notice it), `results` does not prevent the execution of the code but only the display of the results: 
...
However, it is possible to control the overall behavior of the code chunks from  the YAML of the document with the `execute` argument. The `execute` argument must be written on a new line, without indentation, then its arguments must be indented with respect to `execute`: 
...
Instead of importing the images with the typical Markdown code (`![caption](path-to-picture/picture.png)`), they can be imported with the `include_graphics()` function from the `knitr` package [@knitr].
